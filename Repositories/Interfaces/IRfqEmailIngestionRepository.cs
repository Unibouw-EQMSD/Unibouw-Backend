namespace UnibouwAPI.Repositories.Interfaces
{
    public interface IRfqEmailIngestionRepository
    {
        ...
        Task<(DateTime cursorUtc, HashSet<string> processedIdsAtCursor)> GetOrCreateCursorUtcAsync(string pmMailbox);
        Task UpdateCursorUtcAsync(string pmMailbox, DateTime utc, string processedIdsCsv);
        ...
        Task<bool> AnchorExistsAsync(Guid projectId, Guid subcontractorId, Guid rfqId, string pmMailbox, string conversationId);
        ...
        Task<dynamic> GetOfficialProjectDetailsAsync(Guid projectId, Guid rfqId, Guid subId);
