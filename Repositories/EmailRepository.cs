    ...
    public class EmailRepository : IEmail
    {
        ...
        public async Task<bool> SendRfqEmailAsync(EmailRequest request)
        {
            try
            {
                ...
                // Fetch RFQ details
                var rfqQuery = @"
            SELECT RfqID, RfqNumber, DueDate, GlobalDueDate, ProjectID
            FROM Rfq
            ...
            WHERE ProjectID = @ProjectID AND IsDeleted = 0";
                ...
                string projectName = await _connection.QuerySingleOrDefaultAsync<string>(
                    projectQuery, new { ProjectID = rfq.ProjectID });
            ...
            WHERE WorkItemID = @WorkItemID AND IsDeleted = 0";
                ...
                var subQuery = @"
            SELECT SubcontractorID AS Id, EmailID AS Email, ISNULL(Name, 'Subcontractor') AS Name
            FROM Subcontractors 
...
<p>You are invited to submit a quotation for the following work item:</p>

<h4>🔹 Project & RFQ Details</h4>
<ul>
...
<p>
    <a href='{projectSummaryUrl}' 
       ...
       View Project Summary
                    ...
                    {
                        emailBody += "<p><em>Project Summary link has expired.</em></p>";
                    }
